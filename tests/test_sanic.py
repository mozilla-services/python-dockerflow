...
@pytest.mark.parametrize('error,messages', [
    ('connection', {health.ERROR_CANNOT_CONNECT_REDIS: 'Could not connect to '
                                                       'redis: fake'}),
...
def test_redis_check_error(dockerflow_redis, mocker, test_client, error, messages):
    ...
    assert (
        response.json['details']['check_redis_connected']['messages'] ==
        messages
    )
